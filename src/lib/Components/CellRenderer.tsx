...
export const CellRenderer: React.FC<CellRendererProps> = ({
  ...
  const cellToRender = isFocused && currentlyEditedCellRef.current && isMobile ? currentlyEditedCellRef.current : cell; // location 기반 cell을 렌더할지, currentlyEdited인 cell의 current를 렌더할지 선택
  ...
  return (
    ...
    >
      ...
      {!state.enableGroupSelection &&
        ...
        !(isInEditMode && isMobile) && <span className="rg-groupId">{cell.groupId}</span>}
    </div>
  );
};
