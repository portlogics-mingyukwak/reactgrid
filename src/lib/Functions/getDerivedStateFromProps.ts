...
function updateSelections(props: ReactGridProps, state: State): State {
  if (state.selectionMode === "row" && state.selectedIds.length > 0) {
    ...
    state = {
      ...state,
      selectedRanges: [...state.selectedRanges].map((range) => state.cellMatrix.validateRange(range)),
    };
  }
  return state;
}
...
export function appendGroupSelection(props: ReactGridProps, state: State): State {
  return {
    ...state,
