...
export class CellMatrix {
  ...
  getRange(start: Location, end: Location): Range {
    ...
    const rows = this.rows.slice(Math.min(start.row.idx, end.row.idx), Math.max(start.row.idx, end.row.idx) + 1);
  ...
  validateLocation(location: Location): Location {
    const colIdx =
      this.columnIndexLookup[location.column.columnId] ?? Math.min(location.column.idx, this.last.column.idx);
