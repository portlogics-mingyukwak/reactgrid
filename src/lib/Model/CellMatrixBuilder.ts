...
export class CellMatrixBuilder implements ICellMatrixBuilder {
  ...
  fillRowsAndCols(
    ...
    this.cellMatrix.columns = this.cellMatrix.props.columns.reduce<GridColumn[]>((cols, column, idx) => {
  ...
  setRangesToRenderLookup(): CellMatrixBuilder {
    ...
    this.cellMatrix.rows.forEach((row, idy) => {
      row.cells.forEach((cell, idx) => {
        ...
        this.cellMatrix.spanCellLookup[translateLocationIdxToLookupKey(idx, idy)] = { range }; // spanCellLookup에 range(span 속성이 없으면 1x1)를 저장
      });
    });
    ...
    Object.keys(this.cellMatrix.spanCellLookup).forEach((key) => {
      if (!keys.includes(key)) {
        this.cellMatrix.rangesToRender[key] = this.cellMatrix.spanCellLookup[key]; // range로 렌더할 셀들을 spanCellLookup에서 가져옴
      }
    });
    return this;
  }
  ...
  getRangesToRender(range: Range): Range[] {
    const result = range.rows.flatMap((row) => range.columns.map((column) => new Range([row], [column]))); // This results in a two-dimensional array of Range objects, which flatMap then flattens into a single-dimensional array.
    result.shift();
    return result;
  }
  ...
  private getScrollableRange(
    ...
    return new Range(
      this.cellMatrix.rows.slice(topRowsSticky || 0, this.getStickyBottomFirstIdx(topRowsSticky, bottomRowsSticky)),
      this.cellMatrix.columns.slice(
        leftColumnsSticky || 0,
